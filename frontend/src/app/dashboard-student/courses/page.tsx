...
export default function ProjectsPage() {
    ...
    const fetchEnrolledTeachers = async (uid) => {
        try {
            ...
            if (userDocSnap.exists()) {
                ...
                if (userData.userType === 'student' && userData.enrolledUnder) {
                    setEnrolledTeachers(userData.enrolledUnder);
    ...
    const fetchOfflineCourses = async (teacherIds) => {
        try {
            const q = query(
            ...
            {offlineCourses.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {offlineCourses.map(course => (
                        <div key={course.id} className="border rounded-lg p-6 shadow-md bg-white hover:shadow-lg transition-shadow duration-300">
